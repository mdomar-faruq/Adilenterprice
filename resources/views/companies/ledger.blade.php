...
@section('content')
    <div class="container-fluid py-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 d-print-none">
            <div>
                <h2 class="fw-bold mb-1">Company Ledger</h2>
                <p class="text-muted">Statement for <strong>{{ $company->name }}</strong></p>
            </div>
            <div class="d-flex gap-2">
                <button onclick="window.print()" class="btn btn-primary text-white rounded-pill shadow-sm px-4">
        ...
        <div class="row">
            <div class="col-lg-3 d-print-none">
                ...
                <div class="card border-0 shadow-sm rounded-4 p-3">
                    <h6 class="fw-bold small text-uppercase text-muted mb-3">Company Details</h6>
                    <p class="mb-1 small"><strong>Phone:</strong> {{ $company->phone ?? 'N/A' }}</p>
                    <p class="mb-1 small"><strong>Email:</strong> {{ $company->email ?? 'N/A' }}</p>
                    <p class="mb-0 small"><strong>Address:</strong> {{ $company->address ?? 'N/A' }}</p>
                </div>
            </div>

            <div class="col-lg-9 col-print-12">
                <div class="card border-0 shadow-sm border-print-none shadow-print-none rounded-4">
                            ...
                            <tbody>
                                ...
                                @foreach ($ledger as $row)
                                    <tr>
                                        ...
                                        <td class="text-muted small fw-bold">{{ $row->reference ?? 'PAY-' . $row->id }}
                                        </td>
                                        <td class="text-end {{ $row->debit > 0 ? 'text-danger fw-bold' : '' }}">
                                            {{ $row->debit > 0 ? number_format($row->debit, 2) : '-' }}
                                        </td>
                                        <td class="text-end {{ $row->credit > 0 ? 'text-dark fw-bold' : '' }}">
    ...
    <style>
        /* 1. Remove Browser Header/Footer */
        @page {
            size: auto;
            margin: 10mm;
        }
